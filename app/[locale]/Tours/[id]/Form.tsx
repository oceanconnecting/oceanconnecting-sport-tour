...
const FormTour: React.FC<FormProps> = ({ tour }) => {
  ...
  const [reservationDetails, setReservationDetails] = useState<string | null>(
    null
  );
  const [date, setDate] = useState<Date | undefined>(undefined);
  const [totalPrice, setTotalPrice] = useState<number>(0);
  const [showReservationDetails, setShowReservationDetails] =
    useState<boolean>(false); // To toggle reservation modal
  ...
  const handleDateSelect = (selectedDate: Date | undefined) => {
    ...
    if (selectedDate && selectedDate < today) {
      alert(
        "Veuillez sélectionner une date valide (aujourd'hui ou une date future)."
      );
      return;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const totalPrice = PA * adults + PC * children + PB * babies;

    setReservationDetails(
      `Adultes: ${adults}, Enfants: ${children}, Bébés: ${babies}, Date: ${
      ...
      <div className="w-4/5 bg-white p-6 rounded-3xl shadow-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button
            type="submit"
            className={cn(
              " disabled:text-gray-600 disabled:bg-background-400 bg-primary-700 text-text-50"
            )}
            disabled={(adults === 0 && children === 0) || !date}
          >
            ...
            <div className="mt-4 text-gray-700">
              ...
              <div>
                <p className="font-semibold text-lg text-gray-800">
                  {tt("form.Detail_Tariff")} :
                </p>
                {adults > 0 && (
                  <p className="mb-2 grid grid-cols-3 gap-4">
                    <span className="text-sm text-gray-600">
                      {tt("form.Price_Adults")}:
                    </span>
                    <span className="text-sm text-gray-600">
                      {adults}×{tour.newPrice?.priceAdults}{" "}
                    </span>
                    <span className="font-medium text-gray-800">
                      {tour.newPrice?.priceAdults && adults > 0
                        ? (tour.newPrice.priceAdults * adults).toFixed(2) +
                          " dh"
                        : "Price unavailable"}
                    </span>
                  </p>
                ...
                {children > 0 && (
                  <p className="mb-2 grid grid-cols-3 gap-4">
                    <span className="text-sm text-gray-600">
                      {tt("form.Price_Children")}:
                    </span>
                    <span className="text-sm text-gray-600">
                      {children}×{tour.newPrice?.priceChildren}{" "}
                    </span>
                    <span className="font-medium text-gray-800">
                ...
                {babies > 0 && (
                  <p className="mb-2 grid grid-cols-3 gap-4">
                    <span className="text-sm text-gray-600">
                      {tt("form.Price_Babies")}:
                    </span>
                    <span className="text-sm text-gray-600">
                      {babies}×{tour.newPrice?.priceBabies}{" "}
                    </span>
                    <span className="font-medium text-gray-800">
            ...
            <div className="mt-8 flex justify-center">
              {date && (
                <TourReservationComponent
                  date={date}
                  peoples={{ adults, children, babies }}
                  id={tour.id}
                />
              )}
            </div>
